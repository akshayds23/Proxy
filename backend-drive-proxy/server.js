...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (req.method !== "GET" || targetUrl.pathname !== "/stream") {
      res.writeHead(404, { "Content-Type": "text/plain" });
      res.end("Only GET /stream is supported.");
      return;
    }
    const fileId = targetUrl.searchParams.get("id") ?? targetUrl.searchParams.get("fileId");
    if (!fileId) {
